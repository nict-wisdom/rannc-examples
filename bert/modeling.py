...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.megatron_residual_connection = config.megatron_residual_connection
        if not self.megatron_residual_connection:
...
class BertSelfAttention(nn.Module):
    def __init__(self, config, layer_number):
        ...
        self.megatron_residual_connection = config.megatron_residual_connection
        if self.megatron_residual_connection:
            self.LayerNorm = BertLayerNorm(config.hidden_size, eps=1e-12)
        self.scale_query_key = config.scale_query_key
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        # Megatron's scaling
        if self.scale_query_key:
            coeff = self.layer_number
            norm_factor = math.sqrt(coeff * math.sqrt(self.attention_head_size))
            attention_scores = torch.matmul(query_layer / norm_factor, key_layer / norm_factor)
        else:
...
class BertAttention(nn.Module):
    def __init__(self, config, layer_number):
        ...
        self.self = BertSelfAttention(config, layer_number)
...
class BertLayer(nn.Module):
    def __init__(self, config, layer_number):
        ...
        self.attention = BertAttention(config, layer_number)
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList([BertLayer(config, i + 1) for i in range(config.num_hidden_layers)])
...
class BertPreTrainedModel(nn.Module):
    ...
    def init_bert_weights(self, module):
        ...
        elif isinstance(module, BertLayerNorm) or isinstance(module, BertNonFusedLayerNorm):
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids, attention_mask):
        ...
        if self.megatron_residual_connection:
            encoded_layers[-1] = self.LayerNorm(encoded_layers[-1])
